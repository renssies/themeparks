...
class Efteling extends Park {
     ...
     * @param {String} [options.language_code] Laguage used when fetching data
     */
    constructor(options = {}) {
        ...
        this[s_languageCode] = options.language_code || "en"
    ...
    FetchPOIData() {
        return this.MakeRequest({
            ...
            data: {
                "size": 1000,
                "q.parser": "structured",
                "q": `(phrase field=language '${this[s_languageCode]}')`
            }
        }).then((result) => {
            ...
            if (!poiData["baron1898singlerider"]) {
                poiData["baron1898singlerider"] = {
                    name: "Baron 1898 Single-rider"
                }
            }

            if (!poiData["pythonsinglerider"]) {
                poiData["pythonsinglerider"] = {
                ...
                waitData.AttractionInfo.map((item) => {
                    ...
                    if (item.Type == "Attraction" && poiData[item.Id]) {
                        ...
                        if (rideObject) {
                            ...
                            if (item.Timeslot && item.Timeslot.State && item.Timeslot.State == "enabled") {
                                rideObject.WaitTime = -4
                            } 
    ...
    FetchOpeningTimesByMonth(month, year) {
        ...
        return this.HTTP({
            url: `https://www.efteling.com/service/cached/getpoiinfo/${this[s_languageCode]}/${year}/${month}`,
            headers: {
